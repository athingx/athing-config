...
public class PushOpBinder implements OpBinder<OpBind>, Codes {
    ...
    public CompletableFuture<OpBind> bind(OpBindable bindable) {
        return bindable.binding("/sys/%s/thing/config/push".formatted(thing.path().toURN()))
                ...
                .bind((topic, push) -> {
                    ...
                    if (listeners.isEmpty()) {
                        ...
                        thing.op().post(rTopic, OpReply.reply(
                                token,
                    ...
                    try {
                        ...
                        thing.op().post(rTopic, OpReply.success(token));
                    } catch (Throwable cause) {
                        logger.warn("{}/config/push apply failure, token={};config-id={};", path, token, configId, cause);
                        thing.op().post(rTopic, OpReply.reply(token, ALINK_REPLY_PROCESS_ERROR, cause.getLocalizedMessage()));
                    }
